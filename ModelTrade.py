...
CLOSE = 'close'
saved_file = 'data/nifty1.pkl'
#Load data
def read_convert_data(symbol='Nifty'): 
    if symbol == 'Nifty':
        prices = pd.read_csv('data/data.csv',sep=",", skiprows=0, header=0, index_col=0, parse_dates=True, names=['date', 'open', 'high', 'low', CLOSE, 'vol', 'total'])
        prices.to_pickle('data/nifty.pkl')
    return

def load_data(test=False):
    percent_split = .2
    prices = pd.read_pickle(saved_file)
    train_size = int(len(prices) * (1-percent_split))
    x_train = prices.iloc[:train_size,]
    x_test= prices.iloc[train_size:,]
    if test:
        return x_test
    else:
        return x_train
...
def init_state(indata, test=False):
    ...
    if test == False:
        ...
        joblib.dump(scaler, 'data/nifty2.pkl')
    elif test == True:
        scaler = joblib.load('data/nifty2.pkl')
        xdata = np.expand_dims(scaler.fit_transform(xdata), axis=1)
    state = xdata[0:1, 0:1, :]
...
read_convert_data(symbol='Nifty') #run once to read indata, resample and convert to pickle
indata = load_data(test=False)
...
for i in range(epochs):
    if i == epochs-1: #the last epoch, use test data set
        indata = test_data #load_data(test=True)
